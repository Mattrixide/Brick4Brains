...
Usage:
  ...
  - Any pixel coordinate can be converted to world feet and back
...
class FloorPlaneDetector:
    ...
    def __init__(self):
        ...
        self.floor_corners_ft = None  # 4 arena corners in world feet
    ...
    def calibrate_from_pose(self, corners_px, corners_mm, rvec, tvec,
        ...
        """
        # Convert mm to feet
        corners_ft = [[x / MM_PER_FOOT, y / MM_PER_FOOT] for x, y in corners_mm]
    ...
    def _compute_homography(self, pixel_corners, world_corners_ft, rgb_size):
        """Compute homography from pixel corners to world coordinates in feet."""
    ...
    def pixel_to_world(self, px, py):
        """Convert pixel to world coordinates (feet). Returns (x, y) or None."""
    ...
    def world_to_pixel(self, wx, wy):
        """Convert world (feet) to pixel coordinates. Returns (x, y) or None."""
        if not self.calibrated:
            return None
...
def _pose_click_callback(event, x, y, flags, param):
    ...
    wx_mm, wy_mm = world
    wx_ft, wy_ft = wx_mm / MM_PER_FOOT, wy_mm / MM_PER_FOOT
...
def run_single_marker_calibration(cam, camera_matrix, dist_coeffs):
    ...
    while not pose_locked:
        ...
        if r is not None:
            ...
            status = (f"Marker found! Height: {height_mm:.0f}mm "
                      f"({height_mm/MM_PER_FOOT:.1f}ft)  |  SPACE=lock pose")
        else:
    ...
    while True:
        ...
        if r is not None:
            ...
            # Where the calibration THINKS this pixel is (feet)
            ...
            if calib_world and actual_world:
                # Compare in mm
                actual_ft = (actual_world[0] / MM_PER_FOOT,
                             actual_world[1] / MM_PER_FOOT)
                dx = (calib_world[0] - actual_ft[0]) * MM_PER_FOOT
                dy = (calib_world[1] - actual_ft[1]) * MM_PER_FOOT
                ...
                act_label = f"Actual: ({actual_ft[0]:.2f},{actual_ft[1]:.2f})ft"
                cal_label = f"Calib:  ({calib_world[0]:.2f},{calib_world[1]:.2f})ft"
        ...
        if key == 27:  # ESC
            ...
            if actual:
                ...
                print(f"  Added correction point: pixel ({mcx},{mcy}) -> "
                      f"({actual[0]/MM_PER_FOOT:.2f},{actual[1]/MM_PER_FOOT:.2f})ft  "
                ...
                all_ft = [[m[0]/MM_PER_FOOT, m[1]/MM_PER_FOOT] for m in correction_points_mm]
